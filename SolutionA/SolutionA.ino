...
// Declaring Variables and Constants
  // Declaring pins
const int LED = 7;
const int MIC = A0; // NOTE: if MIC is a digital input, replace A0 with 13

  // Declaring sound threshold here
const int THRESHOLD = 500;  // NOTE: if MIC is digital, replace line with: const string THRESHOLD = "HIGH";
...
void setup() {
  pinMode(LED, OUTPUT);
  Serial.begin(9600); // NOTE: if MIC is digital, replace line with: pinMode(MIC, INPUT);
...
void loop() {
  ...
  val = analogRead(MIC); // NOTE: if MIC is digital, replace line with: val = digitalRead(MIC);
  GetBeats(val);
...
unsigned long GetBeats(int sound) // NOTE: if MIC is digital, replace line with: int GetBeats(string sound);
{
  a = millis(); // Getting initial time here so it doesn't update within the loop
  
  while((b) < 60000) // Until 6 seconds have passed
  {
    if(sound > THRESHOLD) // NOTE: if MIC is digital, replace line with: if(sound == THRESHOLD)
    {
      beat++; // increments the number of beats
    }// end if
    
    b = a - millis(); // this will update the length of time stored in b
  }// end while
